module Examples exposing (Model, Msg, init, update, view, subscriptions)

{-| The elm-spreadsheet showcase gallery — one page, many spreadsheets.

This is the **landing** content for the site (see `Main`): a column of independent, editable example
spreadsheets. The page chrome (hero, navbar, footer) is provided by [`Workspace.Site`](Workspace-Site);
this module exposes the gallery as a plain TEA component (`Model`/`Msg`/`init`/`update`/`view`/
`subscriptions`) that the site embeds as its landing.
...
{-| The gallery view (no page chrome — the site supplies the hero and footer). -}
view : Model -> Html Msg
view model =
